...
local OverdriveH = (function()
    ...
    -- Keep a neutral config so the UI can initialize without MarketplaceService
    -- or PlaceId/ProductInfo checks.
...
C.u40.__GetRootPart = function(character)
    ...
    -- The constructor can return the real UI object immediately.
        ...
        -- ============================================================
        -- V8.2 UI DIAGNOSTIC
        -- Runs inside the constructor so every reference below is the
        -- actual object created by this constructor.
        -- ============================================================
        local function V82_UIDiagnostic()
            ...
            local function CheckInstance(Name, Object, ExpectedClass)
                ...
                print("[V8.2][UI-CHECK] " .. Name .. " = OK (" .. Object:GetFullName() .. ")")
                return Object
            end

            local ScreenGui = u907("ScreenGui")
            local Starter = CheckInstance("ScreenGui", ScreenGui)
            CheckInstance("StarterFrame", Starter, "CanvasGroup")
            CheckInstance("MainFrame", MainFrame, "ImageLabel")
            CheckInstance("Title", MainFrame:FindFirstChild("Title"), "TextLabel")
            CheckInstance("TabFrame", TabFrame, "ScrollingFrame")
            CheckInstance("StarterFrame.UIScale", Starter:FindFirstChild("UIScale"), "UIScale")
            CheckInstance("StarterFrame.UICorner", Starter:FindFirstChild("UICorner"), "UICorner")
            CheckInstance("StarterFrame.UIStroke", Starter:FindFirstChild("UIStroke"), "UIStroke")
            CheckInstance("MainFrame.Title.UICorner", MainFrame.Title:FindFirstChild("UICorner"), "UICorner")
            CheckInstance("MainFrame.TabFrame.UICorner", TabFrame:FindFirstChild("UICorner"), "UICorner")
            CheckInstance("MainFrame.TabFrame.UIListLayout", TabFrame:FindFirstChild("UIListLayout"), "UIListLayout")

            if MainFrame.Parent ~= Starter then
                error("[V8.2][UI-CHECK] MainFrame parent mismatch: expected StarterFrame")
            end
            print("[V8.2][UI-CHECK] MainFrame parent = OK")

            if TabFrame.Parent ~= MainFrame then
                error("[V8.2][UI-CHECK] TabFrame parent mismatch: expected MainFrame")
            end
            print("[V8.2][UI-CHECK] TabFrame parent = OK")

            if MainFrame.Title.Parent ~= MainFrame then
