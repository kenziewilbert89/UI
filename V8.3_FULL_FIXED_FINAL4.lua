...
local OverdriveH = (function()
    ...
    function C.u40.Notify(p319, p320, p321)
        ...
        local theme = C.u139
        local accent = (theme and theme.USC) or C._fromRGB(90, 90, 255)
        local background = (theme and theme.UFB) or C._fromRGB(30, 30, 30)
...
C.u40.__GetRootPart = function(character)
    ...
    -- V8.2 FIX: keep shared dispatcher state alive after loading.
        ...
        -- Keep the theme table in a stable constructor-owned reference.
        -- The original local u923 is scoped to an earlier block, while ApplyTheme
        -- is defined later; using C._OverdriveThemes prevents u923 from becoming nil.
        C._OverdriveThemes = u923

        -- Restored outer references lost during the scope-reduction pass.
        local u1233 = C._new3
        local u1234 = C._EasingDirection
        local u1235 = {}
        local u1236 = nil
        local u1237 = u923.Dark or u923.Amethyst
        -- StarterFrame is the tween target used during the opening animation.
        -- Keep a direct constructor reference instead of relying on a missing obfuscated local.
        local v1232 = u921
        ...
        function v926.ApplySetting(p964, p965, p966)
            ...
            elseif p965 == 'Theme' and u932 ~= p966 then
                ...
                elseif p966 == 'Amoled' then
                    -- Standalone build: AMOLED uses the local palette only.
                    -- No remote sprite/theme asset is fetched.
                    u969.ImageRectSize = u973(0, 0)
                    u969.ImageRectOffset = u973(0, 0)
                    u969.ImageTransparency = 0
                    u969.Image = ''
...
print("[V8.2] UI/root:", UI)
    ...
    -- ============================================================
    -- V8.3 PUBLIC API: THEME / NOTIFY / OPEN STATE
    -- ============================================================
    ...
    C.UI.ApplyTheme = function(self, themeName)
        if C.u13(themeName) ~= 'string' then
            return false
        end

        local themes = C._OverdriveThemes
        if type(themes) ~= 'table' then
            return false
        end
        local palette = themes[themeName]
        if not palette then
            warn('[Overdrive-H V8.3] Unknown theme: ' .. tostring(themeName))
            return false
        end

        -- Keep the original ApplySetting/theme API alive.
        self:ApplySetting('Theme', themeName)

        -- Apply to every currently-created UI object immediately.
        u1237 = palette
        u935 = palette
        C.u139 = palette
        u932 = themeName
        _applyThemeTree(u921, palette)

        local notifyGui = C.u155('ScreenGui')
        if notifyGui and typeof(notifyGui) == 'Instance' then
            _applyThemeTree(notifyGui, palette)
        end

        -- Keep the splash/touch UI on the same accent.
        local splash = notifyGui
        if splash and typeof(splash) == 'Instance' then
            local splashButton = splash:FindFirstChild('Shindeiru p\0\1AxueoNe')
            if splashButton and splashButton:IsA('TextButton') then
                splashButton.BackgroundColor3 = palette.UFB
                local stroke = splashButton:FindFirstChildOfClass('UIStroke')
                if stroke then
                    stroke.Color = palette.USC
                end
            end
    ...
    C.UI.SetTheme = function(self, themeName)
    ...
    C.UI.Theme = C._OverdriveThemes or {}
    C.UI.CurrentTheme = function()
        return u932
    end

    -- Default to the first configured theme and make the UI visible.
    C.UI:ApplyTheme(C.u40.ThemeList[1] or 'Dark')
    C.UI:SetOpen(true)

    print('[Overdrive-H V8.3] Standalone module fallbacks enabled')
    print('[Overdrive-H V8.3] Theme / Notify / Open API fixed')
