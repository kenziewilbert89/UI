...
local OverdriveH = (function()
    ...
    function C.u40.Notify(p319, p320)
        ...
        if _ScreenGui2 then
            ...
            -- V8.3 FIX: notification cleanup must run in its own task.
            -- The old implementation stored the cleanup function in the shared
            -- C.u41.Coroutine slot, so nothing reliably started the cleanup and
            -- multiple notifications could overwrite each other.
            C.u130(function()
                C.u131(5)

                -- The notification may already have been removed by the caller.
                if not u331 or not u331.Parent then
                    if u338 then
                        u338:Disconnect()
                        u338 = nil
                    end
                ...
                local v340 = _PlayTween(p319, u331, u328, C.u318)
                if v340 and v340.Completed then
                    v340.Completed:Wait()
                else
                    C.u131(0.5)
                end
...
print("[V8.2] UI/root:", UI)
    ...
    -- ============================================================
    -- V8.3 PUBLIC API FIX: NOTIFY + THEME
    -- ============================================================
    -- Notify originally lived on the internal C.u40 object and was not
    -- exposed by the returned UI object. Expose a safe public wrapper.
    C.UI.Notify = function(self, message)
        return C.u40:Notify(message)
    end

    -- ApplySetting is the library's real theme API. Expose a friendly
    -- SetTheme wrapper so demo/user code can call Window:SetTheme(...).
    C.UI.SetTheme = function(self, themeName)
        if C.u13(themeName) ~= 'string' then
            return false
        end

        local themes = C.u40.ThemeList or {}
        local valid = false

        for i = 1, #themes do
            if themes[i] == themeName then
                valid = true
                break
            end
        end
        ...
        if not valid then
            warn('[Overdrive-H V8.3] Unknown theme: ' .. themeName)
            return false
        end
    ...
    print('[Overdrive-H V8.3] Notify API fixed')
    print('[Overdrive-H V8.3] Theme API fixed')
