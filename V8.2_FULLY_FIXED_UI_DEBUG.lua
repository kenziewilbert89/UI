...
C.u40.__GetRootPart = function(character)
    ...
    -- V8.2 UI diagnostic pass: verify the actual constructor-owned hierarchy
    -- without suppressing errors or creating replacement UI objects.
    local function V82_UIDiagnostic()
        ...
        CheckInstance("StarterFrame", StarterFrame)
        CheckInstance("MainFrame", MainFrame)
        CheckInstance("TabFrame", TabFrame)
        CheckInstance("Title", MainFrame.Title)

        if MainFrame.Parent ~= StarterFrame then
        ...
        if typeof(u906) ~= "table" or type(u906.PlayTween) ~= "function" then
            error("[V8.2][UI-CHECK] PlayTween module registration is invalid")
        end
        print("[V8.2][UI-CHECK] PlayTween module = OK")

        if typeof(JSON) ~= "table" or type(JSON.JSEncode) ~= "function" or type(JSON.JSDecode) ~= "function" then
            error("[V8.2][UI-CHECK] JSON module registration is invalid")
        end
        print("[V8.2][UI-CHECK] JSON module = OK")

        if typeof(Tasker) ~= "table"
            or type(Tasker.IsExistingTask) ~= "function"
            or type(Tasker.MakeTask) ~= "function"
            or type(Tasker.RemoveTask) ~= "function" then
