...
local OverdriveH = (function()
    ...
    -- Preserve the original remote module functionality as an optional
    -- cache/update path. A failed request is logged, not swallowed.
    C.u130(function()
        local _remoteList = C._HttpRequest(C.u40,
            'https://raw.githubusercontent.com/ItsJiDy/Overdrive-H/main/Modules//moduleList.txt')

        if not _remoteList then
            C._Debug(C.u40, 'Failure', 'Remote module list unavailable; using embedded modules.')
            return
        end

        local _remoteNames = C.u77.sp(_remoteList, '\n')
        C._Debug(C.u40, 'Success', 'Remote module list fetched; embedded modules remain active for startup reliability.')

        for i = 1, #_remoteNames do
            local _moduleName = _remoteNames[i]
            if _moduleName and _moduleName ~= '' then
                local _moduleSource = C._HttpRequest(C.u40,
                    'https://raw.githubusercontent.com/ItsJiDy/Overdrive-H/main/Modules//' .. _moduleName .. '.lua')

                if _moduleSource and C._ls then
                    local _chunk, _loadError = C.u15(C._ls, _moduleSource)
                    if _chunk and not _loadError then
                        local _ok, _remoteModule = C.u15(_chunk)
                        if _ok and C._t4(_remoteModule) and _remoteModule.ClassName then
                            C._CheckDirFile2(C.u40, 'Overdrive-H/Modules/' .. _moduleName .. '.lua', _moduleSource)
                            C._Debug(C.u40, 'Success', "Remote module '" .. _moduleName .. "' cached successfully.")
                        else
                            C._Debug(C.u40, 'Failure', "Remote module '" .. _moduleName .. "' returned an invalid module.")
                        end
                    else
                        C._Debug(C.u40, 'Failure', "Remote module '" .. _moduleName .. "' failed to compile: " .. C.u2(_loadError))
                    end
                end
            end
        end
...
C.u40.__GetRootPart = function(character)
    ...
    -- Opening is owned by this constructor and runs exactly once.
    local _openingStarted = false
    if _openingStarted then
        error('Overdrive H V8.2 FIX: duplicate opening initialization detected')
    end
    _openingStarted = true

    u908(0.5)

    -- u906 is the helper/module table (self), not the tween target.
    -- The first argument received by TweenService:Create is StarterFrame.
    local _OpeningTween = _PlayTween5(u906, StarterFrame, v1074, {
    ...
    _PlayTween5(u906, MainFrame.Title, v1269, {
        BackgroundTransparency = 0.35,
        TextTransparency = 0,
    })

    _PlayTween5(u906, MainFrame, v1269, {
        BackgroundTransparency = 0.5,
    })
